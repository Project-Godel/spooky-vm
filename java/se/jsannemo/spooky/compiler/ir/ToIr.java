...
public final class ToIr {
  ...
  private static void initDecl(List<Ast.VarDecl> globals, IrContext ctx)
  ...
  private static void funcDecl(Ast.FuncDecl declaration, boolean extern, IrContext ctx)
      throws ValidationException {
    IrFunction function = new IrFunction();
    function.returnSignature =
        declaration.hasReturnType() ? IrTypes.fromAst(declaration.getReturnType()) : IrTypes.VOID;
    ...
    for (int i = paramsList.size() - 1; i >= 0; i--) {
      ...
      if (cur.vals.containsKey(pName)) {
        throw new ValidationException(
            "Parameter " + pName + " already defined.", param.getPosition());
      }
      ...
      if (ctx.function.returnSignature.equals(IrTypes.VOID)) {
        body.add(
            Ast.Statement.newBuilder()
                .setReturnValue(Ast.ReturnValue.newBuilder().build())
                .build());
      } else {
  ...
  private static void statementList(List<Ast.Statement> body, IrContext ctx)
      throws ValidationException {
    ctx.newScope();
  ...
  private static void varDecl(Ast.VarDecl varDecl, IrContext ctx, boolean isGlobal)
      throws ValidationException {
    ...
    if (res.isPresent()) {
      throw new ValidationException(
          "Variable " + varName + " already declared.", varDecl.getPosition());
    }
    ...
    Ir.Addr varAddr =
        isGlobal
            ? IrAddrs.absStack(cur.spOffset + Conventions.NEXT_STACK.getAbs())
            : IrAddrs.relSp(cur.spOffset);
    ...
    if (!type.equals(valueType)) {
      throw new ValidationException(
          "Invalid type conversion: " + valueType + " to " + type, varDecl.getInit().getPosition());
    }
  ...
  private static void returns(Ast.ReturnValue ret, IrContext ctx) throws ValidationException {
    ...
    if (ret.hasValue()) {
      ctx.function
          ...
          .setCopy(
              Ir.Copy.newBuilder()
                  .setFrom(IrAddrs.relSp(pos))
                  .setTo(IrAddrs.relSp(ctx.function.retValue)));
    }
    ctx.function.newStatement().getJmpAddrBuilder().setAddr(IrAddrs.relSp(ctx.function.retAddress));
  }
  ...
  private static void loop(Ast.Loop loop, IrContext ctx) throws ValidationException {
    ...
    ctx.function.newStatement().getJmpZeroBuilder().setLabel(end).setFlag(IrAddrs.relSp(condAddr));
  ...
  private static void conditional(Ast.Conditional conditional, IrContext ctx)
      throws ValidationException {
    ...
    if (!cond.equals(IrTypes.BOOL)) {
      throw new ValidationException(
          "If-statement condition is not a boolean.", conditional.getPosition());
    }
    Ir.Label elseStart = ctx.function.newLabel();
    Ir.Label elseEnd = ctx.function.newLabel();

    ctx.function.newStatement().getJmpZeroBuilder().setFlag(condAddr).setLabel(elseStart);
  ...
  private static Ir.Type value(Ast.Value value, IrContext ctx) {
    ...
    switch (value.getLiteralCase()) {
      ...
      case INT_LITERAL:
        ...
        return IrTypes.INT;
      default:
        throw new IllegalArgumentException("Unsupported literal type: " + value.getLiteralCase());
    }
  }

  private static Ir.Type functionCall(Ast.FuncCall functionCall, IrContext ctx)
      throws ValidationException {
    ...
    if (func.extern) {
      ...
      ctx.function
          ...
          .getCallBuilder()
          .setName(mName)
          .setSpOffset(ctx.scope.spOffset)
          .setLabel(retLabel);
    }
  ...
  private static Ir.Type reference(Ast.Identifier reference, IrContext ctx)
      throws ValidationException {
    IrContext.Scope cur = ctx.scope;
            ...
            .orElseThrow(
                () ->
                    new ValidationException(
                        "Undefined variable " + reference.getName(), reference.getPosition()));
    ctx.function
        .newStatement()
        .getCopyBuilder()
        .setFrom(ref.getAddress())
        .setTo(IrAddrs.relSp(cur.spOffset));
    ...
    Ir.Value ref =
        cur.resolve(refName)
            .orElseThrow(
                () ->
                    new ValidationException(
                        "Variable " + refName + " does not exist", assign.getPosition()));
  ...
  private static Ir.Type arithmetic(Ast.BinaryOp op, Ast.Expr left, Ast.Expr right, IrContext ctx)
      throws ValidationException {
    IrContext.Scope cur = ctx.scope;
  ...
  private static Ir.Type logical(Ast.BinaryOp op, Ast.Expr left, Ast.Expr right, IrContext ctx)
      throws ValidationException {
    IrContext.Scope cur = ctx.scope;
  ...
  private static Ir.Type comparison(Ast.BinaryOp op, Ast.Expr left, Ast.Expr right, IrContext ctx)
      throws ValidationException {
    IrContext.Scope cur = ctx.scope;
    int addr1 = evalType(left, IrTypes.INT, ctx);
    int addr2 = evalType(right, IrTypes.INT, ctx);
  ...
  private static int evalType(Ast.Expr e, Ir.Type expected, IrContext ctx)
      throws ValidationException {
    int sp = ctx.scope.spOffset;
    Ir.Type type = expr(e, ctx);
    if (!type.equals(expected)) {
      throw new ValidationException(
          "Expression has incorrect type " + type + ", expected " + expected, e.getPosition());
    }
    return sp;
  }
